...
scenarios = {
    ...
    "3. 실험계획법 (Design of Experiments, DOE)": {
        "text": "신소재 배합 공정에서 수율을 극대화(최적화)하기 위한 실험을 설계합니다. 온도와 압력이 수율에 미치는 단독 영향(주효과)과 두 인자가 결합되어 나타나는 시너지(교호작용)를 분석하세요. 분석과정에서 고려된 실험계획법의 원리에 대해 설명하고 결과를 해석하세요. 실험 순서에 따른 환경 오차를 줄이기 위한 개선 방안을 제시하고 개선전/후의 오차를 비교하세요.",
        "files": ["doe_plan.xlsx", "yield_results.csv", "factor_levels.csv"]
    },
    "4. 통계적공정관리 (Statistical Process Control, SPC)": {
        "text": "가죽 시트 제조 공정에서 두께를 측정합니다. 파괴검사 특성을 고려해 계측기 및 작업자의 오차(Gage R&R)가 허용 수준인지 검증하세요. 공정이 관리상태인지 판정하고 그 근거를 제시하세요. 최종적으로 공정능력지수를 산출하고 전체 생산량 대비 불량품의 비율(불량비율)을 모니터링하세요. 장기 또는 단기 공정능력지수를 선정한 이유를 설명하세요.",
        "files": ["leather_thickness.csv", "gage_rr_data.xlsx", "control_chart_points.csv"]
    },
    "5. 구조적 문제해결 방법론": {
        "text": "현재 불량 발생률을 수치로 명확히 정의(As-Is 정량화)하고, 불량률에 영향을 미칠 것으로 예상되는 변수(potential)를 선정하세요. 데이터 분석을 거쳐 불량에 가장 큰 영향을 미치는 소수의 변수(vital few)를 검증하세요. 개선 효과를 검증하기 위한 계획을 제안하세요.",
        "files": ["defect_rate_asis.xlsx", "feature_analysis.csv", "vital_few_analysis.csv"]
    },
    "6. 미니탭 기초 (Minitab Basics)": {
        "text": "제시된 데이터를 데이터프레임 구조에 맞도록 정리하여 미니탭에 입력하세요. 작업자 및 설비별로 데이터를 나누어(층별화) 기초 통계량과 그래프를 확인하고, 상단의 텍스트 결과 영역(세션 창)에 출력된 통계 검정 수치들을 바탕으로 결과를 해석 하세요.",
        "files": ["raw_inspection.xlsx", "stratified_data.csv", "minitab_export.csv"]
    },
    "7. 파이썬 리터러시 (Python Literacy)": {
        "text": "첨부의 파일들에 대해 파이썬을 이용하여 숫자, 문자 등 데이터의 성격(자료형)을 확인하고 리스트 또는 딕셔너리 구조로 저장하세요. 특정 조건에 따라 데이터를 분류하는 조건문(논리 제어)을 작성한 뒤, 엑셀 데이터를 표 형태의 2차원 구조(데이터프레임)로 불러옵니다. 마지막으로 특정 범주별 합계나 평균을 구하기 위해 기준 컬럼으로 데이터를 묶어(GroupBy) 요약하세요.",
